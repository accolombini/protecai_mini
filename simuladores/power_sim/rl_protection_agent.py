...
class ProtectionCoordinationEnv(gym.Env):
    ...
    def _denormalize_action(self, action):
        ...
        for i in range(self.n_reles):
            pickup = action[i*2] * (self.pickup_limits[1] - self.pickup_limits[0]) + self.pickup_limits[0]
            time_delay = action[i*2+1] * (self.time_limits[1] - self.time_limits[0]) + self.time_limits[0]
            actions_real.extend([pickup, time_delay])
        return np.array(actions_real)
    
    def _simulate_fault(self, fault_type="3ph", fault_bus=None):
        """Simula diferentes tipos de falha na rede."""
        ...
        # Simular falha através de impedância baixa
        fault_impedance = {
            "3ph": 0.001,    # Curto trifásico
            "2ph": 0.01,     # Curto bifásico  
            "1ph": 0.1,      # Curto monofásico
            "overload": 10.0 # Sobrecarga
        }
        
        try:
            # Adicionar elemento de falha temporário
            fault_idx = pp.create_load(
                self.net, 
                bus=fault_bus, 
                p_mw=0, 
                q_mvar=0, 
            ...
            # Executar fluxo de carga com falha
            ...
            # Remover elemento de falha
            self.net.load.drop(fault_idx, inplace=True)
    ...
    def _calculate_reward(self, action, currents, voltages, fault_info):
        ...
        if np.any(actions_real[::2] < 60) or np.any(actions_real[::2] > 400):  # Pickup muito baixo/alto
            reward -= 10.0
        if np.any(actions_real[1::2] < 0.2) or np.any(actions_real[1::2] > 1.8):  # Tempo muito baixo/alto
            reward -= 10.0
    ...
    def step(self, action):
        ...
        currents, voltages, fault_bus, fault_type = self._simulate_fault(fault_type)
        
        # Calcular recompensa
        reward = self._calculate_reward(action, currents, voltages, (currents, voltages, fault_bus, fault_type))
    ...
    def render(self, mode='human'):
        ...
        if hasattr(self, 'last_info'):
            print(f"Falha: {self.last_info.get('fault_type', 'N/A')} na barra {self.last_info.get('fault_bus', 'N/A')}")
...
class RLProtectionOptimizer:
    ...
    def train_agent(self, algorithm="PPO", total_timesteps=10000):
        """Treina agente RL."""
        if self.env is None:
            self.create_environment()
        
        print(f"🧠 Iniciando treinamento com {algorithm}...")
    ...
    def evaluate_agent(self, n_episodes=10):
        ...
        print(f"Recompensa Min/Max: {results['min_reward']:.2f} / {results['max_reward']:.2f}")
...
def demonstrate_rl_optimization():
    ...
    protection_devices = {
        "reles": [
            ...
            {"id": "RELE_LINE_1", "element_type": "line", "element_id": 1, "tipo": "OVERCURRENT"},
            {"id": "RELE_TRAFO_0", "element_type": "trafo", "element_id": 0, "tipo": "DIFFERENTIAL"},
            {"id": "RELE_BUS_1", "element_type": "bus", "element_id": 1, "tipo": "VOLTAGE"}
        ]
    }
    ...
    success = optimizer.train_agent(algorithm="PPO", total_timesteps=5000)
