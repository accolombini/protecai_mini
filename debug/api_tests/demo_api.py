...
def demonstrate_features():
    ...
    try:
        ...
        response = requests.post(f"{base_url}/simulation/quick-analysis", json=fault_config)
        if response.status_code == 200:
            analysis = response.json()
            fault_data = analysis.get('fault_analysis', {})
            print(f"   ⚡ Tipo de falta: {fault_data.get('fault_type')}")
            print(f"   🔢 Corrente: {fault_data.get('fault_current', {}).get('magnitude', 0):.0f} A")
            print(f"   📍 Barras afetadas: {len(fault_data.get('affected_buses', []))}")
            print(f"   ⏱️  Tempo de recuperação: {fault_data.get('recovery_time', {}).get('total_recovery_time', 0):.2f} s")
    except Exception as e:
        print(f"   ❌ Erro: {e}")
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print(f"   🏆 Melhor recompensa: {metrics.get('best_overall_reward', 0):.2f}")
    except Exception as e:
        print(f"   ❌ Erro: {e}")
...
def simulate_fault():
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print(f"   Corrente: {fault_data.get('fault_current', {}).get('magnitude', 0):.0f} A")
            print(f"   Impacto: {fault_data.get('system_impact', {}).get('stability')}")
...
def generate_visualization():
    ...
    try:
        ...
        response = requests.post("http://localhost:8000/visualization/generate", json=config)
        if response.status_code == 200:
            data = response.json()
...
def generate_report():
    ...
    try:
        ...
        response = requests.post("http://localhost:8000/visualization/report/generate", json=config)
        if response.status_code == 200:
            data = response.json()
