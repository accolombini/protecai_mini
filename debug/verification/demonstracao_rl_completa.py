...
def executar_pipeline_completo_rl():
    ...
    try:
        ...
        plotar_rede(net, bus_geodata, line_geodata,
                    protection_devices, protection_zones, img_original)
        print("✅ Visualização original salva")
        resultados["etapas"]["visualizacao_original"] = time.time() - \
            inicio_etapa
    ...
    try:
        ...
        resultados["etapas"]["visualizacao_otimizada"] = time.time() - \
            inicio_etapa
    ...
    if optimal_settings:
        ...
        print(
            f"   • Recompensa média: {resultados['performance_rl']['avg_reward']:.2f}")
        ...
        if "settings_comparacao" in resultados:
            ...
            for rele_id in manual.keys():
                if rele_id in rl_opt:
                    ...
                    pickup_melhoria = (
                        (rl_pickup - manual_pickup) / manual_pickup) * 100
                    time_melhoria = (
                        (manual_time - rl_time) / manual_time) * 100
...
if __name__ == "__main__":
    ...
    try:
        ...
        if sucesso:
            ...
            print(
                "💡 Dica: Instale as dependências: pip install stable-baselines3 gymnasium")
