...
def analyze_json_file():
    ...
    print(
        f"📁 Tamanho do arquivo: {file_size:,} bytes ({file_size/1024:.1f} KB)")
    ...
    try:
        ...
        for section in sections:
            if section in data:
                count = len(data[section]) if isinstance(
                    data[section], list) else "não é lista"
        ...
        if "protection_devices" in data:
            ...
            if isinstance(prot_devices, dict):
                for device_type, devices in prot_devices.items():
                    count = len(devices) if isinstance(
                        devices, list) else "não é lista"
