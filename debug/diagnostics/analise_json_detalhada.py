    ...
    try:
        ...
        if 'pandapower_net' in data:
            ...
            try:
                ...
                if 'bus' in net_data:
                    ...
                    print(f"   - Nomes das barras: {[row[0] for row in bus_data['data']]}")
                
                if 'line' in net_data:
                    line_data = json.loads(net_data['line']['_object'])
                    print(f"   - Número de linhas: {len(line_data['data'])}")
                    print(f"   - Nomes das linhas: {[row[0] for row in line_data['data']]}")
                
                if 'trafo' in net_data:
                    trafo_data = json.loads(net_data['trafo']['_object'])
                    print(f"   - Número de transformadores: {len(trafo_data['data'])}")
                    print(f"   - Nomes dos transformadores: {[row[0] for row in trafo_data['data']]}")
                
                if 'load' in net_data:
                    load_data = json.loads(net_data['load']['_object'])
                    print(f"   - Número de cargas: {len(load_data['data'])}")
                    print(f"   - Nomes das cargas: {[row[0] for row in load_data['data']]}")
                
                if 'ext_grid' in net_data:
                    ext_grid_data = json.loads(net_data['ext_grid']['_object'])
                    print(f"   - Número de geradores: {len(ext_grid_data['data'])}")
                    if ext_grid_data['data']:
                        print(f"   - Nome do gerador: {ext_grid_data['data'][0][0]}")
        ...
        if 'protection_devices' in data:
            ...
            if 'reles' in prot_devices:
                ...
                for i, rele in enumerate(reles[:5]):  # Primeiros 5
                    print(f"     * {rele['id']}: {rele['tipo']} - {rele['element_type']} {rele['element_id']}")
        ...
        if 'protection_zones' in data:
            ...
            for zone in zones:
                ...
                print(f"     * Barras: {zone['barras']} (IEEE: {zone['barras_ieee']})")
        ...
        if 'line_geodata' in data:
            ...
            print(f"   - Número de linhas com coordenadas: {len(line_geodata)}")
