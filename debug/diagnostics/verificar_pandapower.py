        ...
        for element in elements:
            if element in net_data:
                try:
                    ...
                    print(f"   - {element.upper()}: {rows} elementos, {cols} colunas")
                    
                    if rows > 0:
                        print(f"     * Colunas: {element_data['columns']}")
                        print(f"     * Primeira linha: {element_data['data'][0]}")
                        
                        # Verificações específicas
                        if element == 'bus':
                            bus_names = [row[0] for row in element_data['data']]
                            print(f"     * Nomes das barras: {bus_names}")
                            
                        elif element == 'line':
                            line_names = [row[0] for row in element_data['data']]
                            print(f"     * Nomes das linhas: {line_names}")
                            
                        elif element == 'trafo':
                            trafo_names = [row[0] for row in element_data['data']]
                            print(f"     * Nomes dos transformadores: {trafo_names}")
                            
                        elif element == 'load':
                            load_names = [row[0] for row in element_data['data']]
                            print(f"     * Nomes das cargas: {load_names}")
                            
                        elif element == 'ext_grid':
                            gen_names = [row[0] for row in element_data['data']]
        ...
        try:
            ...
            for line in line_data['data']:
                ...
                if 0 <= from_bus < bus_count and 0 <= to_bus < bus_count:
                    ...
                    print(f"     * ERRO: Linha {line[0]} conecta barras inválidas: {from_bus} -> {to_bus}")
            ...
            if trafo_count > 0:
                ...
                for trafo in trafo_data['data']:
                    ...
                    if 0 <= hv_bus < bus_count and 0 <= lv_bus < bus_count:
                        print(f"     * {trafo_name}: {hv_bus} -> {lv_bus} (OK)")
                    else:
                        print(f"     * {trafo_name}: {hv_bus} -> {lv_bus} (ERRO)")
            else:
        ...
        try:
            ...
            if load_count > 0:
                ...
                for load in load_data['data']:
                    ...
                    if 0 <= bus < bus_count:
                        print(f"     * {load_name}: Bus {bus}, P={p_mw} MW, Q={q_mvar} MVar (OK)")
                    else:
                        print(f"     * {load_name}: Bus {bus} (ERRO - barra inválida)")
                
                print(f"   - Potência total: P={total_p:.1f} MW, Q={total_q:.1f} MVar")
            else:
...
if __name__ == "__main__":
    ...
    json_file = Path(__file__).parent / "simuladores" / "power_sim" / "data" / "ieee14_protecao.json"
