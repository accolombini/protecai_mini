...
class ProtecAIPipelineTester:
            ...
            if response.status_code == 200:
                ...
                if data.get("status") == "healthy":
                    ...
                    self.log_result("Health Endpoint", False,
                                    f"Status não saudável: {data.get('status')}")
            else:
                self.log_result("Health Endpoint", False,
    ...
    def test_zones_configuration(self):
        ...
        try:
            response = requests.get(
                f"{self.base_url}/api/v1/protection-zones/zones/detailed-configuration", timeout=10)
            if response.status_code == 200:
                data = response.json()
                ...
                if "zone_configuration" in data:
                    ...
                    if "zona_1_primaria" in zones:
                        ...
                        if z1.get("selectivity_index", 0) > 90:
                            self.log_result(
                                "Zones Configuration", True, f"Zona 1 seletividade: {z1.get('selectivity_index')}%")
                            return True
                        else:
                            self.log_result(
                                "Zones Configuration", False, f"Seletividade baixa: {z1.get('selectivity_index')}%")
                    else:
                        self.log_result("Zones Configuration",
                                        False, "Zona 1 não encontrada")
                else:
                    self.log_result("Zones Configuration", False,
    ...
    def test_realtime_status(self):
        ...
        try:
            response = requests.get(
                f"{self.base_url}/api/v1/protection-zones/zones/real-time-status", timeout=10)
            if response.status_code == 200:
                data = response.json()
                ...
                if "zones_status" in data and "summary" in data:
                    ...
                    if total_zones > 0:
                        self.log_result("Real-time Status", True,
                                        f"{total_zones} zonas monitoradas")
                        return True
                    else:
                        self.log_result("Real-time Status", False,
                                        "Nenhuma zona monitorada")
                else:
                    self.log_result("Real-time Status", False,
            ...
            if response.status_code == 200:
                ...
                if "fault_simulation" in data and "simulation_id" in data:
                    ...
                    if primary_ops:
                        ...
                        self.log_result(
                            "Fault Simulation", True, f"{device_name} operou em {op_time*1000:.1f}ms")
                        return True
                    else:
                        self.log_result("Fault Simulation", False,
                                        "Nenhum dispositivo primário operou")
                else:
                    self.log_result("Fault Simulation", False,
                                    "Estrutura de resposta inválida")
            else:
    ...
    def test_network_visualization(self):
        ...
        try:
            response = requests.get(
                f"{self.base_url}/api/v1/protection-zones/visualization/complete", timeout=10)
            if response.status_code == 200:
                data = response.json()
                ...
                if "network_topology" in data:
                    ...
                    if len(buses) >= 14 and len(lines) >= 10:
                        self.log_result("Network Visualization", True,
                                        f"{len(buses)} barras, {len(lines)} linhas")
                        return True
                    else:
                        self.log_result(
                            "Network Visualization", False, f"Dados incompletos: {len(buses)} barras, {len(lines)} linhas")
                else:
                    self.log_result("Network Visualization",
                                    False, "Estrutura de dados inválida")
            else:
                self.log_result("Network Visualization", False,
                                f"Status code: {response.status_code}")
        except Exception as e:
    ...
    def test_device_management(self):
        ...
        try:
            response = requests.get(
                f"{self.base_url}/api/v1/protection-zones/zones", timeout=10)
            if response.status_code == 200:
                data = response.json()
                ...
                if isinstance(data, list) and len(data) > 0:
                    ...
                    if total_zones > 0:
                        self.log_result(
                            "Device Management", True, f"{total_zones} zonas de proteção encontradas")
                        return True
                    else:
                        self.log_result("Device Management",
                                        False, "Nenhuma zona encontrada")
                else:
                    self.log_result("Device Management", False,
                                    "Estrutura de dados inválida")
            else:
                self.log_result("Device Management", False,
                                f"Status code: {response.status_code}")
    ...
    def test_standards_compliance(self):
        ...
        try:
            response = requests.get(
                f"{self.base_url}/api/v1/protection-zones/standards/compliance-monitoring", timeout=10)
            if response.status_code == 200:
                data = response.json()
                ...
                if "compliance_summary" in data:
                    ...
                    if overall_score > 80:
                        self.log_result("Standards Compliance", True,
                                        f"Score de conformidade: {overall_score}%")
                        return True
                    else:
                        self.log_result("Standards Compliance",
                                        False, f"Score baixo: {overall_score}%")
                else:
                    self.log_result("Standards Compliance", False,
                                    "Dados de conformidade não encontrados")
            else:
                self.log_result("Standards Compliance", False,
                                f"Status code: {response.status_code}")
        except Exception as e:
    ...
    def run_all_tests(self):
        ...
        if self.failed == 0:
            ...
            print(
                f"⚠️  {self.failed} teste(s) falharam. Revisar antes da demonstração.")
            return False
