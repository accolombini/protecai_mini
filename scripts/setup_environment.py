...
class EnvironmentSetup:
    ...
    def analyze_dependencies(self):
        ...
        for package, required_version in requirements:
            ...
            if package_lower in self.installed_packages:
                ...
                if installed_version == required_version:
                    ...
                    print(
                        f"⚠️ {package}: {installed_version} → {required_version} (ATUALIZAR)")
                    self.incompatible_packages.append(
        ...
        print(
            f"   • Pacotes para atualizar: {len(self.incompatible_packages)}")

    def install_packages(self, force_reinstall: bool = False):
        """Instala pacotes faltantes e incompatíveis."""
        to_install = self.missing_packages + \
            (self.incompatible_packages if force_reinstall else [])
        ...
        for category_name, category_packages in categories:
            ...
            if category_name == "🔧 OUTROS":
                ...
                for _, cat_pkgs in categories[:-1]:
                    installed_in_categories.update(
                        [p.lower() for p in cat_pkgs])
    ...
    def verify_critical_packages(self):
        ...
        critical_tests = [
            ...
            ("Pandas",
             "import pandas as pd; print(f'Pandas {pd.__version__} OK')"),
            ("PandaPower",
             "import pandapower as pp; print(f'PandaPower {pp.__version__} OK')"),
            ...
            ("Stable-Baselines3",
             "import stable_baselines3; print(f'SB3 {stable_baselines3.__version__} OK')"),
            ("Gymnasium",
             "import gymnasium; print(f'Gymnasium {gymnasium.__version__} OK')"),
        ]
    ...
    def generate_report(self):
        ...
        # Estatísticas
        total_required = len(self.missing_packages) + \
            len(self.incompatible_packages)
        success_rate = (len(self.success_installs) /
                        max(total_required, 1)) * 100
