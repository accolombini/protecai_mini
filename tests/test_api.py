...
def test_api():
    ...
    try:
        response = requests.get(
            f"{BASE_URL}/network/topology", timeout=TIMEOUT)
        print(f"   Status: {response.status_code}")
        data = response.json()
    ...
    try:
        response = requests.get(
            f"{BASE_URL}/protection/devices", timeout=TIMEOUT)
        print(f"   Status: {response.status_code}")
        data = response.json()
    ...
    try:
        response = requests.get(
            f"{BASE_URL}/protection/zones", timeout=TIMEOUT)
        print(f"   Status: {response.status_code}")
        data = response.json()
    ...
    try:
        ...
        if response.status_code == 200:
            data = response.json()
            fault_current = data.get(
                'fault_analysis', {}).get('fault_current', {})
            print(
                f"   Corrente de falta: {fault_current.get('magnitude', 0):.2f} A")
            print(
                f"   Barras afetadas: {len(data.get('fault_analysis', {}).get('affected_buses', []))}")
    except Exception as e:
        print(f"   ❌ Erro: {e}")
    ...
    try:
        response = requests.get(
            f"{BASE_URL}/simulation/templates", timeout=TIMEOUT)
        print(f"   Status: {response.status_code}")
        data = response.json()
    ...
    try:
        response = requests.get(
            f"{BASE_URL}/rl/config/default", timeout=TIMEOUT)
        print(f"   Status: {response.status_code}")
        data = response.json()
    ...
    try:
        response = requests.get(
            f"{BASE_URL}/visualization/templates", timeout=TIMEOUT)
        print(f"   Status: {response.status_code}")
        data = response.json()
        print(f"   Templates disponíveis: {len(data.get('templates', []))}")
    ...
    try:
        response = requests.get(
            f"{BASE_URL}/visualization/formats", timeout=TIMEOUT)
        print(f"   Status: {response.status_code}")
        data = response.json()
    ...
    try:
        response = requests.get(
            f"{BASE_URL}/simulation/statistics", timeout=TIMEOUT)
        print(f"   Status: {response.status_code}")
        data = response.json()
            ...
            for i in range(10):  # Máximo 10 tentativas
                ...
                if response.status_code == 200:
                    ...
                    if status == "completed":
                        ...
                        response = requests.get(
                            f"{BASE_URL}/simulation/results/{simulation_id}")
                        if response.status_code == 200:
                            results = response.json()
                            print(
                                f"   Análise de faltas: {len(results.get('results', {}).get('fault_analysis', []))}")
                            print(
                                f"   Ações de proteção: {len(results.get('results', {}).get('protection_response', []))}")
                            print(
                                f"   Recomendações: {len(results.get('results', {}).get('recommendations', []))}")
                        break
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            for i in range(15):  # Máximo 15 tentativas
                ...
                response = requests.get(
                    f"{BASE_URL}/rl/training/status/{training_id}")
                if response.status_code == 200:
                    status_data = response.json()
                    status = status_data.get('status')
                    ...
                    print(
                        f"   Status: {status} | Progresso: {progress:.1f}% | Recompensa: {current_reward:.2f}")
                    ...
                    if status == "completed":
                        ...
                        if response.status_code == 200:
                            ...
                            actions = prediction.get('prediction', {}).get(
                                'recommended_actions', [])
